...
def get_snapshot_writing_instruction(round_index: int = 1) -> str:
    ...
    if _is_chinese_output():
        return (
            ...
            "「本轮新增」：【仅】记录本轮首次提出、上轮快照中尚未出现的新论点，需包含具体数据或逻辑链。"
            "若上轮已提过某论点，本轮必须填写不同内容；若本轮无全新论点，则写本轮对既有论点的最新量化补充或反驳进展；\n"
            "「关键反驳」：必须针对对手【本轮最新发表的论点】，而非泛指其一贯立场。"
            "格式：「[对手角色]本轮提出[具体主张]，但[我方反驳+数据]，因此难以支撑其[推论]」。"
            "每轮对手论点不同，反驳内容必须随之更新，严禁与上轮快照重复；\n"
            "「待验证」：下轮需跟踪的具体指标、事件及触发条件，如「Q2铜价是否企稳8000美元上方；36元支撑位的成交量变化；美联储6月利率决议」。\n"
            "【关键约束】：将你的上轮快照与本轮快照逐字段对比，确保「本轮新增」和「关键反驳」有实质性差异，否则视为无效快照。\n"
    ...
    return (
        ...
        "'New this round': record ONLY arguments introduced for the FIRST TIME this round that did NOT appear in your previous snapshot. "
        "If the previous snapshot already covered a point, you MUST write something different here — "
        "e.g. a new data point, updated metric, or argument progression from this round;\n"
        "'Key rebuttal': MUST target the opponent's argument from THIS round specifically, not their general stance. "
        "Format: '[Opponent] argued this round that [X], but [rebuttal + data], undermining their conclusion that [Y].' "
        ...
        "KEY CONSTRAINT: Compare your previous snapshot with this one field by field — "
